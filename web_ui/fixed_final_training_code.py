...
# =============================================================================
# 第3步：您的Roboflow项目配置（修复版）
# =============================================================================

# 🎯 您的真实项目信息（使用URL友好的项目名称）
ROBOFLOW_CONFIG = {
    "api_key": "BwTemPbP39LHLFH4teds",
    "workspace": "malaysian-food-detection",
    "project": "malaysian-food-detection",  # 使用URL友好的名称
    "version": 2,                            # ✅ 您有2个版本
...
print(f"🍜 您的数据集包含: Nasi Lemak, Wantan Mee, Chee Cheong Fun, Char Kway Teow")
...
# 训练配置（针对您的57张图片优化）
TRAINING_CONFIG = {
    "model_type": "yolov8n",      # 小模型，适合小数据集
    "epochs": 100,                # 训练轮次
    "batch_size": 8 if device == "cuda" else 4,  # 小批次，适合小数据集
    "learning_rate": 0.01,        # 学习率
    "img_size": 640,              # 图像大小
    "patience": 30,               # 早停耐心值（增加，因为数据集小）
    "save_period": 10,            # 保存周期
    "device": device
}
...
try:
    ...
    project_names_to_try = [
        "malaysian-food-detection",      # URL友好版本
        "Malaysian Food Detection",      # 原始版本
        "malaysian_food_detection",      # 下划线版本
    ]
    
    project = None
    dataset = None
    
    for project_name in project_names_to_try:
        try:
            print(f"🔍 尝试项目名称: {project_name}")
            project = workspace.project(project_name)
            
            # 尝试不同的版本
            for version_num in [2, 1]:  # 先尝试版本2，再尝试版本1
                try:
                    print(f"🔍 尝试版本: {version_num}")
                    dataset = project.version(version_num).download("yolov8")
                    print(f"✅ 成功！使用项目名称: {project_name}, 版本: {version_num}")
                    break
                except Exception as version_error:
                    print(f"❌ 版本 {version_num} 失败: {version_error}")
                    continue
            ...
            print(f"❌ 项目名称 '{project_name}' 失败: {project_error}")
            continue
    
    if not dataset:
        # 如果所有方法都失败，尝试直接使用项目ID
        print("\n🔄 尝试使用项目ID直接访问...")
        try:
            # 直接使用项目ID创建URL
            project_url = f"https://app.roboflow.com/malaysian-food-detection/malaysian-food-detection-wy3kt"
            print(f"🔗 项目URL: {project_url}")
            
            # 尝试使用项目ID
            project = workspace.project("malaysian-food-detection-wy3kt")
            dataset = project.version(2).download("yolov8")
            print("✅ 使用项目ID成功！")
            
        except Exception as id_error:
            print(f"❌ 项目ID方法也失败: {id_error}")
            raise Exception("无法访问您的Roboflow项目，请检查项目权限和名称")
    
    print(f"✅ 您的真实数据集下载完成！")
    print(f"📁 数据集路径: {dataset.location}")
    print(f"📊 数据集名称: {dataset.name}")
    print(f"📊 数据集版本: {dataset.version}")
    print(f"🍜 食物类别: Nasi Lemak, Wantan Mee, Chee Cheong Fun, Char Kway Teow")
    ...
    print("\n🔄 尝试替代方案...")
    print("💡 如果自动下载失败，您可以:")
    print("1. 访问您的Roboflow项目页面")
    print("2. 手动下载YOLOv8格式的数据集")
    print("3. 上传到Colab进行训练")
...
try:
    ...
    # 分析您数据集中的食物
    your_foods = ["Nasi Lemak", "Wantan Mee", "Chee Cheong Fun", "Char Kway Teow"]
...
summary = {
    ...
    "dataset_info": {
        ...
        "food_categories": ["Nasi Lemak", "Wantan Mee", "Chee Cheong Fun", "Char Kway Teow"]
    },
    "model_results": {
...
print(f"食物类别: {', '.join(summary['dataset_info']['food_categories'])}")
